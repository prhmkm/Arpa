...
namespace ArpaBackend.Core.Helpers
{
    public static class SecurityHelpers
    {
        ...
        private static string GenerateKey(string pass)
        {
            var key = Convert.ToBase64String(Encoding.UTF8.GetBytes(pass));
            var str = 0;
            while (key.Length < 32)
            {
                key += str;
