...
using static ArpaBackend.Domain.DTOs.UserDTO;
...
namespace PrivatizationBackend.Api.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public IActionResult UsersList()
        {
            ...
            {
                var userId = HttpContext.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier).Value;
                List<User> users = _service.User.GetAllUsersExceptCurrent(Convert.ToInt32(userId));
                ...
                return Ok(new { TimeStamp = DateTime.Now, ResponseCode = HttpStatusCode.OK, Message = "لیست کاربران با موفقیت ارسال شد", Value = new { response = UserList }, Error = new { } });
            }
            catch (Exception ex)
            {
                return Ok(new { TimeStamp = DateTime.Now, ResponseCode = HttpStatusCode.InternalServerError, Message = "خطای داخلی سرور رخ داده است", Value = new { }, Error = new { Response = ex.ToString() } });
